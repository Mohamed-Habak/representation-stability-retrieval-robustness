...
def export_all_tables(csv_dir, table_dir):
    ...
    gee_log.to_excel(os.path.join(table_dir, "TableS1_GEE_Log_Resolution_Sensitivity.xlsx"), index=False)
    descriptive.to_excel(os.path.join(table_dir, "TableS2_Descriptive_Statistics.xlsx"), index=False)
    anova_norm.to_excel(os.path.join(table_dir, "TableS3_ANOVA_Residual_Normality.xlsx"), index=False)
    paired_norm.to_excel(os.path.join(table_dir, "TableS4_Paired_Diff_Normality.xlsx"), index=False)
    anova.to_excel(os.path.join(table_dir, "TableS5_RM_ANOVA.xlsx"), index=False)
    posthoc.to_excel(os.path.join(table_dir, "TableS6_PostHoc_Comparisons.xlsx"), index=False)
    retrieval_accuracy.to_excel(os.path.join(table_dir, "TableS7_Retrieval_Accuracy.xlsx"), index=False)
    gee.to_excel(os.path.join(table_dir, "TableS8_GEE_Retrieval_Odds_Ratios.xlsx"), index=False)
    mediation.to_excel(os.path.join(table_dir, "TableS9_Mediation_Analysis.xlsx"), index=False)
    per_res_corr.to_excel(os.path.join(table_dir, "TableS10_Per_Resolution_Correlation.xlsx"), index=False)
    mcnemar.to_excel(os.path.join(table_dir, "TableS11_McNemar_Results.xlsx"), index=False)
